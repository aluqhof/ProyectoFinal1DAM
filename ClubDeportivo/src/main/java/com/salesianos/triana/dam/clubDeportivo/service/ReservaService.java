...
public class ReservaService extends BaseServiceImp<Reserva, Long, ReservaRepositorio> {
	...
	public boolean isHoraDisponible(LocalTime horaReserva, LocalDate fechaReserva, int id) {
		for (Reserva reserva : this.findAll()) {
			if (reserva.getPista().getId() == id && reserva.getFecha_reserva().equals(fechaReserva)
					&& reserva.getHora_reserva().equals(horaReserva)) {
				return false;
			}
		}
		return true;
	...
	public void calcularPrecio(Reserva reserva) {
		if (reserva != null && reserva.getHora_reserva() != null) {
			double div = 100;
	...
	public List<Reserva> findReservasProximaSemanaYPista(int idPista) {
		...
		List<Reserva> reservas = repositorio.findByFechaReservaEntreYPista(inicioProximaSemana, finProximaSemana,
				idPista);
		return reservas;
	}
	...
	public List<Reserva> findReservasHoyYDeporte(int idDeporte, LocalDate dia) {
